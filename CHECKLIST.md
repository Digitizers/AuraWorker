...
- [x] Text domain is `aurawp` in all `__()`, `_e()`, `esc_html__()`, `esc_html_e()` calls
- [x] Text domain in plugin header matches: `Text Domain: aurawp`
- [x] No remaining references to old text domain `aura-worker` in PHP strings
...
- [x] Validate at: https://wordpress.org/plugins/developers/readme-validator/
...
- [ ] Re-run PCP — confirm 0 errors, only trademark warning remaining
...
- [x] Build clean ZIP — GitHub Action attaches `aurawp.zip` to release automatically
